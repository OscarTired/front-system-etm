  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      ...
      >
        ...
        {mode === "save" && (
          <div className="shrink-0 space-y-3 border-b border-white/5 px-5 py-3">
            <label className="flex flex-col gap-1">
              ...
              <input
                ...
                className="h-9 rounded-lg border-none bg-white/5 px-3 text-sm text-neutral-100 outline-none focus:bg-white/8 focus:ring-1 focus:ring-white/20"
        ...
        {mode === "open" && (
          <div className="shrink-0 border-b border-white/5 px-5 py-3">
            <label className="group flex cursor-pointer items-center gap-3 rounded-xl bg-white/4 p-3 transition-colors hover:bg-white/8">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/5 text-neutral-400 group-hover:text-white">
                <HardDrive size={18} />
              </div>
              <div className="flex min-w-0 flex-col">
                <span className="text-sm font-medium text-neutral-200">Abrir archivo local</span>
                <span className="truncate text-[11px] text-neutral-500">.json — ProjectFile v1 o v2</span>
              </div>
              <input
                type="file"
          ...
          {loading && items.length === 0 ? (
            ...
            <ul className="flex flex-col gap-1.5">
              {items.map((item) => {
                ...
                return (
                  <li key={item.id} className="flex items-center gap-2 rounded-xl bg-white/3 p-2.5 transition-colors hover:bg-white/6">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/5 text-neutral-400">
                      <Cloud className="h-4 w-4" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm font-medium text-neutral-100">{label}</div>
