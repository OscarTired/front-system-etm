...
export async function parsePdf(fileName: string, data: ArrayBuffer): Promise<CadData> {
  ...
  if (typeof window !== "undefined" && !GlobalWorkerOptions.workerSrc) {
    // Mismo patrón que ya usan tus otros exportadores de PDF
    // (export-production-sheet-pdf.ts, etc.) para las fuentes de
    // pdfmake: cargar el asset versionado desde unpkg en vez de pelear
    // con el bundler para servir el worker como estático.
