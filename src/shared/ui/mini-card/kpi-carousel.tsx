...
export function KpiCarousel({ cards, summary, defaultExpanded = false }: Props) {
  ...
  return (
    <div className="w-full">
      {/* Contenedor colapsado con transición por Grid (sin saltos de altura) */}
      <div
        className={cn(
            ...
            <div className="flex min-w-0 flex-1 items-center justify-end gap-4 tablet:gap-8">
              {summary.values.map((v) => (
                <div key={v.label} className="min-w-0 text-right">
                  ...
                  <p className="text-lg font-bold leading-tight" style={{ color: textColor }}>
                    {v.value}
                  </p>
                </div>
              ))}
      ...
      {/* Contenedor expandido con transición por Grid y padding lateral corregido */}
      <div
        className={cn(
          "grid transition-all duration-300 ease-in-out",
